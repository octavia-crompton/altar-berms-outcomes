...
\title{ Berm structural stability and vegetation response diverge in southwestern US rangelands}
% Structural Integrity vs. Vegetation Response: contrasting outcomes for berms in southwestern US Rangelands
...
\begin{abstract} 
Earthen berms are commonly implemented in semi-arid rangelands to control water runoff, increase soil moisture, and mitigate soil erosion. The effectiveness and stability of these berms is influenced by various environmental factors, including geomorphic position, soil properties, and vegetation. 


This study examines how berm structural condition relates to physiographic features -- including landform, parent material, soil development, and berm morphology -- using statistical analyses, using, as a case study, 550 berms in the Altar Valley, southern Arizona.
We also quantify berm impacts on proximal vegetation, using remotely-sensed vegetation greenness (specifically, the Soil Adjusted Vegetation Index, SAVI), 
use diffeence in greenness between upslope and downslope to define berm `effectiveness' (differences in vegetation greenness between upslope and downslope positions)

The results indicate that berms on fan terraces, and in areas with stronger soil development, exhibit greater stability and enhanced upslope vegetation greenness.
...
Factors such as parent material, surface slope, and berm length significantly affect berm structural condition.
...
\label{sec:Altar}
In this study, we focus on water spreader berms that were originally constructed to capture hillslope runoff and increase infiltration upslope from these structures. We use an inventory of mapped berms in the semi-arid, 247,000 ha Altar Valley of southern Arizona, USA \cite{nichols2021landscape}.  \citet{nichols2021landscape} dataset of mapped berm locations that have been classified as intact, breached, or flanked, based on a combination of satellite imagery and aerial LiDAR data (2011 and 2016; Pima County Regional Flood Control District). This study focuses on a subset of 550 berms distributed across a variety of landforms in the Altar Valley (map figure).  

\subsection{Statistical analysis}
We hypothesize that berm structural condition and vegetative response to berms are related to watershed physical features. Chi-square ($\chi^2$) tests of independence were used to test the hypothesis that berm condition and vegetative impacts (response variables) are related to geomorphic landform, parent material, soil characteristics, berm length, and surface slope (predictor variables) (Table~\ref{tab:variables}). 
Response variables and predictor variables were organized as categorical data. 
Berm structural condition is described as 1 of 3 categories (intact, breached, or flanked). Landform is described as 1 of 3 categories (flood plains, stream terraces, or fan terraces, hills). Parent material is described as 1 of 3 categories (moderately fine/coarse textured alluvium, mixed alluvium, or alluvium derived from granite and/or schist, mixed alluvium).
...
A balanced comparison of berm structural condition between stable and unstable landforms, stratified by parent material composition and cohesion, showed marked differences in the performance of berms. 
For stable landforms (fan terraces and stream terraces), 13\% of berms were breached, 37\% were flanked, and 49\% remained intact. 
