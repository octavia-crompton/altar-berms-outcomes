...
\figentry%
  ...
  {Six predictors of vegetation response shown in a 2×3 grid. Row 1: berm length (threshold 60 m), slope class, and landform. Row 2: soil texture (n > 100 per class), B horizon presence, and flow accumulation (optimal threshold 2k cells, Fisher p = 0.0001139). Vegetation response is significantly associated with slope class (p < 0.001), landform (p < 0.001), and soil texture (p < 0.001), but not with berm length alone. Steep-slope berms and flood-plain berms show the highest rates of vegetation response.}
%% FIG_7_END
...
\figentry%
  ...
  {Six predictors of structural condition (Intact vs Degraded) shown in a 2×3 grid. Row 1: berm length (threshold 60 m), slope class, and landform. Row 2: soil texture (n > 100 per class), B horizon presence, and flow accumulation (optimal threshold 1k cells, Fisher p = 2.065e-16).}
%% FIG_8_END
...
\figentry%
  {fig6}%
  {fig6_veg_response_by_condition_texture.png}%
  {2026-04-18 14:50}%
